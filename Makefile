...
# ================================================
# 🧩 Pre-Change Verification & Safe Push Suite (v2.8)
# ================================================

.PHONY: verify-code verify-docs verify-before-change guide-safe-push safe-push

# --- Проверка Python-стека ---
verify-code: ## Проверка black, isort, flake8
	@echo "🧩 Step 1: Verifying Python stack (black, isort, flake8)..."
...
verify-docs: ## Проверка и автофиксация markdown/yaml-документов
	@echo "🧾 Step 2: Verifying documentation (markdown/yaml)..."
	@npx markdownlint-cli2 --fix "docs/**/*.md" || true
	@pre-commit run markdownlint --all-files || true
	@pre-commit run yamllint --all-files || true
	@echo "✅ Documentation verification complete (autofixed if needed)."

# --- Полный pre-change аудит ---
verify-before-change: ## Полный аудит перед изменениями
	@echo "🔍 Starting pre-change verification suite..."
	@python3 --version
	@echo "🧩 Checking dependencies..."
	@pip check || echo "⚠️ Some dependencies may be outdated (ignored)."
	@echo "🧩 Checking Docker environment..."
	@docker compose config >/dev/null 2>&1 && echo "✅ Docker Compose valid." || echo "⚠️ Docker Compose not found, skipping."
	@$(MAKE) verify-code
	@$(MAKE) verify-docs
	@echo "🧩 Running pre-commit audit..."
	@pre-commit run --all-files || true
	@echo "✅ Pre-change verification finished successfully."

# --- Safe Push & Verification Guide ---
guide-safe-push: ## 📘 Показать гайд по безопасному пушу и проверке изменений
	@echo "🧩 Opening Safe Push & Verification Guide (v2.8)..."
	@if [ -f docs/SAFE_PUSH_AND_VERIFICATION_GUIDE_v2.8.md ]; then \
		cat docs/SAFE_PUSH_AND_VERIFICATION_GUIDE_v2.8.md | less; \
	else \
		echo "❌ Guide file not found: docs/SAFE_PUSH_AND_VERIFICATION_GUIDE_v2.8.md"; \
		echo "💡 Hint: recreate it with 'make fix-docs-lint' or copy from repository."; \
	fi

# --- Safe Push ---
safe-push: ## 🚀 Полный пуш с верификацией и коммитом
	@echo "🧩 Running safe push procedure..."
	@$(MAKE) verify-before-change
	@git add -A
	@read -p "📝 Enter commit message: " msg; \
	git commit -m "$$msg" || true
	@git push origin HEAD || echo "⚠️ Push failed — check branch or network."
...
# ===============================================================
# 🧪 TEST CLEAN SUITE (v2.9 — "Генеральная чистка")
# ===============================================================

.PHONY: test-clean test-smoke integration-test test-ci-clean

test-clean: ## 🧹 Полный прогон pytest с очисткой окружения
	@echo "🧹 [Clean] Полный запуск pytest..."
	...
	@bash -c 'PYTHONPATH=backend pytest backend/app/tests -v --disable-warnings --maxfail=1 --tb=short' || ( \
		echo "❌ Ошибка при общем прогоне тестов"; \
		exit 1; )
	@echo "✅ [Clean] Все тесты успешно пройдены."

test-smoke: ## 🚦 Быстрые smoke-тесты после чистки
	@echo "🚦 [Smoke] Проверка ключевых маршрутов..."
	@PYTHONPATH=backend \
	pytest -m smoke backend/app/tests \
		-v --disable-warnings --maxfail=1 --tb=short || ( \
			echo "❌ Smoke-тесты не пройдены!"; \
			python3 backend/app/utils/notify_telegram.py '🚨 Smoke-tests failed (Clean Protocol v2.9)' || true; \
			exit 1; )
	@echo "✅ [Smoke] Все smoke-тесты успешно пройдены."

integration-test: ## 🌐 Интеграционные тесты под Clean Protocol
	@echo "🌐 [Integration] Запуск интеграционного набора..."
	@PYTHONPATH=backend \
	pytest -m integration backend/app/tests \
		-v --disable-warnings --maxfail=1 --tb=short || ( \
			echo "❌ Интеграционные тесты не пройдены."; \
			python3 backend/app/utils/notify_telegram.py '🚨 Integration tests failed (KAD/Pravo/Telegram)' || true; \
			exit 1; )
	@echo "✅ [Integration] Все интеграционные тесты успешно завершены."

test-ci-clean: ## 🤖 CI + Clean Protocol (pytest + smoke + integration)
	@echo "🤖 [CI Clean] Запуск полного пайплайна (pytest + smoke + integration)..."
	...
	@echo "📢 Отправка статуса в Telegram..."
	@python3 backend/app/utils/notify_telegram.py "✅ CI Clean Protocol v2.9 завершён успешно" || true
	@echo "✅ [CI Clean] Полный тестовый цикл завершён."
...
# ============================================
# ⚙️ DevOps & CI Docs Chain — v2.9.7
# ============================================

lint-md: ## 🧾 Проверка Markdown-файлов
	@npx markdownlint-cli2 --fix "docs/**/*.md" || true
	@echo "✅ Markdown проверен и исправлен."

docs-check: ## 📚 Проверка документации (Markdown + YAML)
	@echo "🔍 Проверка форматирования документации..."
	@npx markdownlint-cli2 --fix "docs/**/*.md" "artifacts/**/*.md" || true
	@pre-commit run markdownlint --all-files || true
	@pre-commit run yamllint --all-files || true
	@echo "✅ Проверка документации завершена."

ci-docs: ## 🤖 Полный CI-док цикл (lint-md + docs-check + progress-auto-push)
	@echo "🚀 Запуск полного цикла проверки документации..."
	@$(MAKE) lint-md
	@$(MAKE) docs-check
	@$(MAKE) progress-auto-push
	@echo "✅ CI Docs Flow завершён успешно (v2.9.7)."

verify-before-change: ## 🔍 Полный аудит перед изменениями
	@echo "🔍 Pre-change verification v2.9.7..."
	@pre-commit run --all-files || true
	@$(MAKE) lint-md
	@$(MAKE) docs-check
	@pytest -m smoke --disable-warnings -q || true
	@echo "✅ Проверка завершена — можно коммитить."

check-all: ## 🧭 Полный аудит (код + доки + CI)
	@echo "🧩 Выполняется полный аудит проекта (v2.9.7)..."
	@$(MAKE) verify-before-change
	@$(MAKE) ci-docs
...
help: ## 📖 Все команды
	...
	@echo "\n🧩 Советы:"
	@echo "   • make verify-before-change  → Полная проверка перед изменениями"
	@echo "   • make safe-push              → Безопасный пуш с верификацией"
	@echo "   • make guide-safe-push        → Гайд по безопасным изменениям"
	@echo "   • make help                   → Этот экран"
.PHONY: \
	backup-makefile \
